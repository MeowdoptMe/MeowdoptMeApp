...
urlpatterns = [
    ...
    path('<int:pk>/remove/', PhotoAlbumDetail.as_view(), name='photo_album_remove'), #delete
    path('<int:pk>/edit/', PhotoAlbumDetail.as_view(), name='photo_album_edit'), #put
    path('<int:pk>/photos/', PhotoList.as_view(), name='photo_list'), #get
    path('<int:id>/photos/<int:pk>/', PhotoDetail.as_view(), name='photo_detail'), #get
]